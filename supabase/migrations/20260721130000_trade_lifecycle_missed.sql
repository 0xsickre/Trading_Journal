-- Trade lifecycle: planned / missed statuses + miss review fields.

ALTER TABLE public.tj_positions
  ADD COLUMN IF NOT EXISTS miss_reason text,
  ADD COLUMN IF NOT EXISTS missed_at timestamptz;

-- Existing plans without fills were incorrectly stored as open.
...
-- miss_reason option list for existing users
...
CROSS JOIN (values
  ('miss_reason', 'Miss Reason', 'Risk', 12)
...
WHERE list_id IN (SELECT id FROM public.tj_option_lists WHERE key = 'miss_reason');

INSERT INTO public.tj_option_items (user_id, list_id, value, label, sort_order)
SELECT l.user_id, l.id, v.value, v.value, v.ord
FROM (values
  ('miss_reason', 'No fill', 0),
  ('miss_reason', 'Price ran away', 1),
  ('miss_reason', 'Setup invalidated', 2),
  ('miss_reason', 'News / event', 3),
  ('miss_reason', 'Discretion', 4),
  ('miss_reason', 'Session ended', 5),
  ('miss_reason', 'Other', 6)
) AS v(list_key, value, ord)
JOIN public.tj_option_lists l ON l.key = v.list_key;

-- New-user seed: add miss_reason list + items
...
begin
  ...
  if not exists (select 1 from public.tj_option_lists where user_id = target) then
    ...
    from (values
      ...
      ('miss_reason','Miss Reason','Risk',12),
      ('emotion','Emotion','Psychology',13),
      ('discipline','Discipline / Behavior','Psychology',14),
      ('rules_followed','Rules Followed','Psychology',15),
      ('mistake','Mistake','Psychology',16)
    ) as v(key, label, category, sort_order);
